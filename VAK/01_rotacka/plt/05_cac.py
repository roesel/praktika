...
#x,y,z # z s errorama
d = np.loadtxt(script_name()+".txt", converters = {0: cf, 1: cf, 2: cf})
x,y,z = d[:,0],d[:,1],d[:,2]

#dfit = d
#popt,pcov = curve_fit(func, dfit[:,0], dfit[:,1])
...
plt.clf()
plt.plot(x,y,'x', label=u"Vypočítané hodnoty", ms=15)
plt.plot(x,z,'+', label=u"Naměřené hodnoty", ms=15)
#plt.plot(x,y_fit, label=u"Fitovaná funkce")

# Kosmetika
plt.grid()
plt.legend(loc=2)
plt.ylabel(r"C [cm$^3$/s]")
plt.xlabel(r"p [Pa]")
#plt.xlim(0,30)
#plt.ylim(1,9)

#plt.text(5.1, 6.1, u'f(x)='+get_pretty_param(ps[0], pe[0])+'x+'+get_pretty_param(ps[1], pe[1])+'', fontsize=12)
